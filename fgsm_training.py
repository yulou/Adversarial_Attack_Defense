...
def evaluate(model, test_loader, device, epsilon):
    """
...
def adversarial_training(
    ...
    for epoch in range(num_epochs):
        ...
        clean_acc, clean_loss_avg, adv_acc, adv_loss_avg = evaluate(model, test_loader, device, epsilon)
        ...
        if test_acc > best_test_acc:
            ...
            torch.save(model.state_dict(), f'best_finetuned_fgsm_ep{epsilon:.2f}.pt')
...
num_epochs = 10
epsilon = 16/255  # Maximum perturbation
...
torch.save(model.state_dict(), f"trained_fgsm_ep_{epsilon:.2f}.pt")
